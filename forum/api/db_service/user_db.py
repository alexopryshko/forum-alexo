...
class User:
    ...
    def save(self):
        ...
        try:
            cursor.execute("""INSERT INTO Users (
                                        ...
                                        isAnonymous,
                                        about
                              ) VALUES (%s, %s, %s, %s, %s);""", (self.username, self.email, self.name, self.isAnonymous, self.about))
            connection.commit()
            cursor.close()
    ...
    def update(self):
        cursor = connection.cursor()
        try:
            cursor.execute("""UPDATE Users SET name= %s, about = %s WHERE email = %s""", (self.name, self.about, self.user_id))
            connection.commit()
            result = True
        except IntegrityError:
            connection.rollback()
    ...
    def follow(follower, followee):
        follower_id = User.get_inf(follower)
        if follower_id is None:
            return None
        followee_id = User.get_inf(followee)
        ...
        cursor.close()
        return User.get_inf(True, follower=follower)
    ...
    def unfollow(follower, followee):
        ...
        try:
            ...
            connection.rollback()
        cursor.close()
        return User.get_inf(True, id=follower)
